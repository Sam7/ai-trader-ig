...
* **Deployment continuity**: before a production replacement, the staged worker records each market's final-bar checkpoint and publishes a verified SQLite snapshot; failure blocks the restart. After the new worker has established its configured stream subscriptions, it repairs only the bounded restart gap through the same rate- and allowance-aware recovery coordinator. The deployment workflow requires a `Succeeded` continuity report; an IG-confirmed closed or suspended no-bar interval is recorded as evidence rather than treated as a gap.
